...
# 메모장에 500 step 별 fid 점수를 기록하도록 작성.
# 무엇이 문제가 되었는지는 모르겠으나 매 반복마다 메모장 내용이 완전히 초기화 되는 문제가 발생함.
# 해결하려 여러가지를 시도해봤으나 실패.
# 모델의 성능에 영향을 미치는 것은 아님으로 문제 해결을 위한 시도는 하지 않음.
...
noise_div = 1.5 # 노이즈 표준 편차. 논문에서는 0.1 
n_critic = 1 # 논문에서는 5
alpha = 0.0002 # learning rate 논문에서는 0.00005
max_epoch = 1600
...
def gan () :
    ...
    def fid(G):
        #FID를 위한 setting
        image_num = 667
        noise = torch.randn(image_num, n_noise).to(DEVICE)*noise_div

        directory_path = f"samples{file_num}/generated_images"
        if not os.path.exists(directory_path):
            os.makedirs(directory_path)
        ...
        # FID 점수 계산
        fid_value = fid_score.calculate_fid_given_paths([directory_path, data_folder], batch_size=64, device=DEVICE, dims=2048)
        print('FID:', fid_value)
    ...
    D_labels = torch.ones([batch_size, org_channel]).to(DEVICE) # Discriminator Label to real
    D_fakes = torch.zeros([batch_size, org_channel]).to(DEVICE) # Discriminator Label to fake
    ...
    with open(f'{directory_path}/readme.txt', 'a') as File:
        for score in fids :
            File.write(f'{step}:fid score\t{score}')
            step += 500
